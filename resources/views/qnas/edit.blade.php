...
@section('content')
    <div class="max-w-7xl mx-auto mt-24">
        <div class="max-w-5xl mx-auto">
            <form class="w-full" action="{{route('frees.update', $qna->id)}}" method="post">
                @csrf
                @method('put')
                <p>
                    ...
                    <input id="title" type="text" name="title"
                           ...
                           placeholder="제목을 입력하세요." value="{{$qna->title}}">
                </p>
                <p class="mt-8">
                    <label for="story"></label>
                    <textarea id="story" name="story"
                              class="border-2 border-blue-200 rounded-md focus:border-gray-600 focus:outline-none
                              resize-none w-full h-screen px-1 py-1" placeholder="내용을 입력하세요.">{{$qna->story}}</textarea>
                </p>
