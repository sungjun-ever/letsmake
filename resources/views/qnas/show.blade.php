...
@section('content')
    <div class="max-w-7xl mx-auto mt-24">
        <div class="max-w-5xl mx-auto">
            <p class="border-b-2 border-blue-400 py-1 text-xl font-semibold overflow-hidden overflow-ellipsis whitespace-nowrap">{{$qna->title}}</p>
            <div class="mt-8 h-screen">{{$qna->story}}</div>
            @auth()
                @if(auth()->user()->id == $qna->user_id)
                    <div class="mt-6 text-right">
                        ...
                        <form class="inline-block" action="{{route('qnas.destroy', $qna->id)}}" method="post">
                            @csrf
                            @method('delete')
                ...
                @else
                    <div class="mt-6 text-right">
                        <a href="{{route('qnas.reply', $qna->id)}}">
                            <i class="xi-pen-o hover:text-blue-400 cursor-pointer">
                                <button>답변</button>
                            </i>
                        </a>
                    </div>
...
{{--                --}}{{--     댓글 작성       --}}
{{--                @auth()--}}
{{--                    <form action="{{route('frees.comments.store', $task->id)}}" method="post" class="mt-8">--}}
{{--                        @csrf--}}
